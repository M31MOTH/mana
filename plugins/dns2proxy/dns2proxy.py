...
RESOLVCONF = "%s/resolv.conf" % CONF_DIR
victim_file = "%s/victims.cfg" % CONF_DIR
nospoof_file = "%s/nospoof.cfg" % CONF_DIR
nospoofto_file = "%s/nospoofto.cfg" % CONF_DIR
specific_file = "%s/spoof.cfg" % CONF_DIR
dominios_file = "%s/domains.cfg" % CONF_DIR
transform_file = "%s/transform.cfg" % CONF_DIR
...
class Dns2proxy(object):
    ...
    def __init__(self,
        ...
        self.arg_ip1 = arg_ip1
        self.arg_ip2 = arg_ip2
        self.arg_ips = arg_ips
        self.silent = silent
    ...
    def start(self):
        ...
        self.proc = Process(target=self._start, args=(self.interface,
                                self.noforward,
                                self.arg_ip1,
                                self.arg_ip2,
                                self.arg_ips,
    ...
    @staticmethod
    def _start(interface, noforward, arg_ip1, arg_ip2, arg_ips, silent, adminIP):
...
if __name__ == '__main__':
    ...
    dns2proxy(configs['interface'],
            noforward=configs['noforward'],
            arg_ip1=configs['ip1'],
            arg_ip2=configs['ip2'],
            arg_ips=configs['ips'],
            silent=configs['silent'],
            adminIP=configs['adminIP'])
