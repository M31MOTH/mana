...
def main():
    ...
    desc = ("Glenn's Firelamb: This tool will parse pcap files or listen "
            "on an interface for cookies. Cookies get saved to a Firefox "
            "cookies.sqlite file - one cookie file per observed device.  "
            "(glenn@sensepost.com)")
    ...
    parser.add_option("-f", "--file",
                      dest="fname",
                      help="Specify pcap file to read")

    parser.add_option("-i", "--interface",
                      dest="iface",
                      help="Specify interface to listen on")

    parser.add_option("-p", "--ip_logging",
                      action="store_true",
                      dest="log_by_ip",
                      default=False,
                      help=("Create cookie file per IP address. "
                          "Default is per device MAC address  "))

    parser.add_option("-l", "--launch_firefox",
                      dest="launch_ff",
                      action = "store_true",
                      default=False,
                      help="Launch Firefox profiles for the saved cookies")

    parser.add_option("-s", "--karma_sslstrip",
                      dest="sslstriplog", 
                      default=None,
                      help="SSLStrip log file")

    parser.add_option("-t", "--karma_sslsplit",
                      dest="sslsplitdir",
                      default=None,
                      help="Directory of SSLSplit log files")

    (options, args) = parser.parse_args()

    firelamb(fname=options.fname,
             iface=options.iface,
             log_by_ip=options.log_by_ip,
             launch_ff=options.launch_ff,
             sslstriplog=options.sslstriplog,
